 ...
 * version: 0.4  (2026-06-11) — instant start (no 1s delay), idempotent (T=0 early-load safe),
 *                self-caching config, cheap-bail observer
 * author: @ViktorTabori
 *
 * How to install it:
 *  - go to page [[roam/js]]
...
window.ViktorDateformatter = (function(){
	// Idempotent: if an instance is already running (e.g. instant-roam early-loaded us at T=0 to kill
	// the date flash), KEEP it — stopping+restarting would roll dates back to native and re-flash.
	if (window.ViktorDateformatter && window.ViktorDateformatter.isStarted && window.ViktorDateformatter.isStarted())
		return window.ViktorDateformatter;
	if (window.ViktorDateformatter && typeof window.ViktorDateformatter.stop === 'function') window.ViktorDateformatter.stop();

	// Config priority: live ViktorOpts (set by the loader, post-boot) -> cache (for a T=0 early-load,
	// before ViktorOpts exists) -> defaults. We self-cache below so the next early-load has it.
	var O = window.ViktorOpts || {}, _cache = {};
	try { _cache = JSON.parse(localStorage.getItem('Viktor_dfcfg') || '{}'); } catch (e) {}

	var dateformat = O.dateformat || _cache.dateformat || 'YYYY.MM.DD EEE';
	...
	*/
	var observer,
		...
		months = O.nameMonths || _cache.months || ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'],
		monthsShort = O.nameMonthsShort || _cache.monthsShort || ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
		days = O.nameDays || _cache.days || ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'],
		daysShort = O.nameDaysShort || _cache.daysShort || ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];

	// cheap-bail guard for the observer: skip subtrees with no month name (avoids querySelectorAll
	// on every node Roam adds during boot — keeps the early-load observer from adding boot jank)
	var MONTH_RE = /January|February|March|April|May|June|July|August|September|October|November|December/;

	// self-cache the resolved config so a future T=0 early-load (before ViktorOpts exists) matches it
	try { localStorage.setItem('Viktor_dfcfg', JSON.stringify({ dateformat: dateformat, months: months, monthsShort: monthsShort, days: days, daysShort: daysShort })); } catch (e) {}

	// start immediately (was setTimeout(start, 1000) — that 1s delay made the native-date flash linger)
	start();
	...
	function processMutation(mutation) {
		for (var i=0; i<mutation.addedNodes.length; i++) {
			...
			// cheap-bail: an element subtree with no month name can't contain a date to reformat
			if (n.nodeType === 1 && n.textContent && !MONTH_RE.test(n.textContent)) continue;
			changeDate(n);
		}
	}
